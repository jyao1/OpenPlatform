...
class FileChecker:
    def __init__(self):
        self.sourceRoot = ""
        self.includeCoreList = ["\\CryptoPkg\\", "\\FatBinPkg\\", "\\FatPkg\\", "\\IntelFrameworkModulePkg\\", "\\IntelFrameworkPkg\\", "\\IntelFspPkg\\", "\\IntelFspWrapperPkg\\", "\\MdeModulePkg\\", "\\MdePkg\\", "\\NetworkPkg\\", "\\PcAtChipsetPkg\\", "\\PerformancePkg\\", "\\SecurityPkg\\", "\\ShellBinPkg\\", "\\ShellPkg\\", "\\SourceLevelDebugPkg\\", "\\UefiCpuPkg\\"]
    ...
    def GetModulePathFromMakefile (self, file):
        # Split file to get dir (filePath[0]) and file (filePath[1])
        # C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86\IA32\FatPkg\EnhancedFatDxe\Fat\Makefile
        # [0] : C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86\IA32\FatPkg\EnhancedFatDxe\Fat
        # [1] : Makefile
        filePath = os.path.split(file)

        # Split filePath[0] to get file dir (dirPath[0]) and inf (dirPath[1])
        # C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86\IA32\FatPkg\EnhancedFatDxe\Fat
        # [0] : C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86\IA32\FatPkg\EnhancedFatDxe
        # [1] : Fat
        dirPath = os.path.split(filePath[0])

        # Split dirPath[0] to get build prefix (strPath[0]) and source path (strPath[1])
        # C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86\IA32\FatPkg\EnhancedFatDxe
        # [0] : C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86
        # [1] : FatPkg\EnhancedFatDxe
        strPath = dirPath[0].split("\\X64\\")
        if (len(strPath) == 1):
            strPath = dirPath[0].split("\\IA32\\")

        # Split strPath[0] to get build source (buildPath[0]) and build dir (buildPath[1])
        # C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86
        # [0] : C:\home\Edk-II
        # [1] : NT32IA32\DEBUG_VS2015x86
        buildPath = strPath[0].split("\\Build\\")

        # join build source and source path
        # [0] : C:\home\Edk-II
        # [1] : FatPkg\EnhancedFatDxe
        # C:\home\Edk-II\FatPkg\EnhancedFatDxe
        finalDir = os.path.join (buildPath[0], strPath[1])

        # record sourceRoot = c:\home\Edk-II
        self.sourceRoot = buildPath[0]
        if (self.IsInExclusiveList(strPath[1]) == False):
            self.AddPackageList(strPath[1], self.usedPackageList)

        return finalDir

    def AddPackageList(self, file, packageList):
        packageName = file.split("\\")
        packagePath = os.path.join (self.sourceRoot, packageName[0])
            ...
            while 1:
                ...
                if (cmp (headFile, "") != 0) and self.IsIncludeFile(headFile):
                    ...
                    if (self.IsInExclusiveList(finalPath) == False) and (finalPath not in self.usedIncludeFileList):
                        ...
                        self.AddPackageList (headFile, self.usedPackageList)
    ...
    def GetAllPackageList(self):
        if (len(self.allPackageList) == 0):
            ...
            for file in self.allFileList:
                self.AddPackageList(file[prefixLength + 1:], self.allPackageList)
...
#
def usage():
    print "Usage: \n\tCheckCodeBase <Build Dir> used|unused|all|delete_unused"
    ...
    print "       delete_unused - delete unused library, modules, include file"
    ...
    print "For Example: \n\tCheckCodeBase C:\home\Edk-II\Build\NT32IA32\DEBUG_VS2015x86 used"
